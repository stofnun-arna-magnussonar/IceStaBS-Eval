...
def load_config_yaml(config_filepath: str) -> Dict[str, Any]:
...
RULES = load_rules_json(
    os.path.join(
        os.path.dirname(os.path.dirname(current_file_path)), "data/IceStaBS-SP.json"
    )
)
CONFIG = load_config_yaml(os.path.join(os.path.dirname(current_file_path), "eval.yml"))
