...
from .globals import RULES, CONFIG

# read the config from eval.yaml
...
def get_original_set(set_nr: int) -> List[str]:
...
def get_standardized_set(set_nr: int) -> List[str]:
    if set_nr not in [1, 2, 3]:
        raise ValueError(f"Invalid example set number: {set_nr}")
    set_nr = set_nr - 1
    return [RULES[key].examples[set_nr].standardized_sentence for key in RULES.keys()]
